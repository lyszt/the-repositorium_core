...
import node_child_process, { fork } from "node:child_process";
...
function isTransferable(value) {
    return null != value && "object" == typeof value && kTransferable in value && kValue in value;
}
function isMovable(value) {
    return isTransferable(value) && true === value[kMovable];
}
...
try {
    ...
    } else if ("win32" === platform) throw new Error();
    else {
        const cores = node_os.cpus().filter(function(cpu, index) {
    ...
    amount = node_os.cpus().length;
}
if (0 === amount) amount = node_os.cpus().length;
...
var ProcessWorker = class {
    ...
    postMessage(message, transferListItem) {
        ...
        if (this.port) this.port.on("message", (message$1)=>this.send({
                ...message$1,
                source: "port",
                __tinypool_worker_message__: __tinypool_worker_message__
            }));
        return this.send({
            ...message,
            source: "pool",
    ...
    on(event, callback) {
        return this.process.on(event, (data)=>{
            if ("error" === event) return callback(data);
            if (!data || !data.__tinypool_worker_message__) return this.channel?.postMessage?.(data);
            if ("pool" === data.source) callback(data);
            else if ("port" === data.source) this.port.postMessage(data);
        });
    }
    once(event, callback) {
...
function hasUnref(stream) {
    return null != stream && "unref" in stream && "function" == typeof stream.unref;
}
const cpuCount = amount;
function onabort(abortSignal, listener) {
    if ("addEventListener" in abortSignal) abortSignal.addEventListener("abort", listener, {
        once: true
    });
...
var ArrayTaskQueue = class {
    ...
    remove(task) {
        ...
        node_assert.notStrictEqual(index, -1);
...
var TaskInfo = class extends AsyncResource {
    ...
    constructor(task, transferList, filename, name, callback, abortSignal, triggerAsyncId, channel){
        ...
        this.channel = channel;
        if (isMovable(task)) {
            if (null == this.transferList) this.transferList = [];
...
var AsynchronouslyCreatedResource = class {
    ...
    markAsReady() {
        ...
        node_assert(null !== listeners);
        this.onreadyListeners = null;
        for (const listener of listeners)listener();
    }
    isReady() {
        return null === this.onreadyListeners;
    }
    onReady(fn) {
        if (null === this.onreadyListeners) return void fn();
...
var WorkerInfo = class extends AsynchronouslyCreatedResource {
    ...
    constructor(worker, port, workerId, freeWorkerId, onMessage, filename, teardown){
        ...
        this.sharedBuffer = new Int32Array(new SharedArrayBuffer(2 * Int32Array.BYTES_PER_ELEMENT));
    }
    ...
    _handleResponse(message) {
        ...
        if (0 === this.taskInfos.size) this.unref();
    }
    postTask(taskInfo) {
        node_assert(!this.taskInfos.has(taskInfo.taskId));
        const message = {
    ...
    processPendingMessages() {
        const actualResponseCount = Atomics.load(this.sharedBuffer, 1);
        if (actualResponseCount !== this.lastSeenResponseCount) {
            this.lastSeenResponseCount = actualResponseCount;
            let entry;
            while(void 0 !== (entry = receiveMessageOnPort(this.port)))this._handleResponse(entry.message);
        }
    }
    isRunningAbortableTask() {
        if (1 !== this.taskInfos.size) return false;
        const [first] = this.taskInfos;
        const [, task] = first || [];
        return task?.abortSignal !== null;
    }
    currentUsage() {
        if (this.isRunningAbortableTask()) return 1 / 0;
...
var ThreadPool = class {
    ...
    constructor(publicInterface, options){
        ...
        if (void 0 !== options.maxThreads && this.options.minThreads >= options.maxThreads) this.options.minThreads = options.maxThreads;
        if (void 0 !== options.minThreads && this.options.maxThreads <= options.minThreads) this.options.maxThreads = options.minThreads;
        if ("auto" === options.maxQueue) this.options.maxQueue = this.options.maxThreads ** 2;
        else this.options.maxQueue = options.maxQueue ?? kDefaultOptions.maxQueue;
        this.workerIds = new Map(new Array(this.options.maxThreads).fill(0).map((_, i)=>[
                i + 1,
                true
            ]));
    ...
    _addNewWorker() {
        ...
        const worker = "child_process" === this.options.runtime ? new ProcessWorker() : new ThreadWorker();
        worker.initialize({
        ...
        worker.on("message", (message$1)=>{
            if (true === message$1.ready) {
                port1.start();
                if (0 === workerInfo.currentUsage()) workerInfo.unref();
    ...
    _onWorkerAvailable(workerInfo) {
        ...
        if (0 === workerInfo.taskInfos.size && this.workers.size > this.options.minThreads) workerInfo.idleTimeout = setTimeout(()=>{
            node_assert.strictEqual(workerInfo.taskInfos.size, 0);
    ...
    runTask(task, options) {
        ...
        if (null == filename) filename = this.options.filename;
        if (null == name) name = this.options.name;
        if ("string" != typeof filename) return Promise.reject(Errors.FilenameNotProvided());
        ...
        if (this.taskQueue.size > 0) {
            ...
            if (this.taskQueue.size >= totalCapacity) if (0 === this.options.maxQueue) return Promise.reject(Errors.NoTaskQueueAvailable());
        ...
        if (null !== workerInfo && workerInfo.currentUsage() > 0 && signal) workerInfo = null;
        let waitingForNewWorker = false;
        if ((null === workerInfo || workerInfo.currentUsage() > 0) && this.workers.size < this.options.maxThreads) {
            this._addNewWorker();
    ...
    shouldRecycleWorker(taskInfo) {
        ...
        if (!this.options.isolateWorkers && void 0 !== this.options.maxMemoryLimitBeforeRecycle && (taskInfo?.workerInfo?.usedMemory || 0) > this.options.maxMemoryLimitBeforeRecycle) return true;
        return false;
    }
    pendingCapacity() {
    ...
    _maybeDrain() {
        if (0 === this.taskQueue.size && 0 === this.skipQueue.length) this.publicInterface.emit("drain");
    }
    async destroy() {
...
var Tinypool = class extends EventEmitterAsyncResource {
    #pool;
    constructor(options = {}){
        if (void 0 !== options.minThreads && options.minThreads > 0 && options.minThreads < 1) options.minThreads = Math.max(1, Math.floor(options.minThreads * cpuCount));
        if (void 0 !== options.maxThreads && options.maxThreads > 0 && options.maxThreads < 1) options.maxThreads = Math.max(1, Math.floor(options.maxThreads * cpuCount));
        super({
            ...options,
            name: "Tinypool"
        });
        if (void 0 !== options.minThreads && void 0 !== options.maxThreads && options.minThreads > options.maxThreads) throw new RangeError("options.minThreads and options.maxThreads must not conflict");
    ...
    static move(val) {
        if (null != val && "object" == typeof val && "function" != typeof val) {
            if (!isTransferable(val)) val = types.isArrayBufferView(val) ? new ArrayBufferViewTransferable(val) : new DirectlyTransferable(val);
            markMovable(val);
        }
        return val;
    }
