import { logger, writeFile, mkdir, rm, join, isDebugMode } from "../../4942.js";
...
const rsbuildPluginSSG = ({ routeService, config })=>({
        ...
        async setup (api) {
            api.onBeforeBuild(()=>{
                let htmlTemplate = '';
                let hasError = false;
                const missingIndexHtmlMessage = 'SSG requires an `index.html` as entry, but this file is not emitted successfully in the web target.';
                ...
                api.processAssets({
                    ...
                    if (hasError) return;
                    await renderPages(routeService, config, mainCjsAbsolutePath, htmlTemplate, emitAsset, distPath);
